      ...
      >
        ...
        <div className="position-relative">
          <div className="d-flex align-items-center">
            ...
            <div>
              <h3 className="mb-1 fw-bold">Financial Metrics</h3>
              <p className="mb-0 opacity-75">Cost vs budget for each project</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="table table-hover mb-0 custom-table">
            <thead>
              <tr className="gradient-header-table text-white">
                ...
                <th className="border-0 fw-semibold py-3 px-4">Cost</th>
                <th className="border-0 fw-semibold py-3 px-4">Budget</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="3" className="text-center py-5 text-muted">
                    <div className="d-flex flex-column align-items-center">
                      <i className="bi bi-graph-down display-1 mb-3 opacity-25"></i>
                      <h5 className="mb-2">No financial data found</h5>
                ...
                data.map((item, index) => (
                  <tr key={index} className="table-row-custom">
                    ...
                    <td className="py-3 px-4 text-danger fw-bold">
                      {item.cost?.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}
                    </td>
                    <td className="py-3 px-4 text-success fw-bold">
                      {item.budget?.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}
                    </td>
                  </tr>
                ))
              )}
